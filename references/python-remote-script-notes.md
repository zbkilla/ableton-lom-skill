...
- **Follow actions** — verified Live 12.4: no `follow_action_*` member on `Clip`, `ClipSlot`, `Scene`, or `Song`. The Launch box's Follow Action settings can't be read or set from **any** host (the LOM lacks them, so Max/JS is no better). (Clip *launch* settings — `launch_mode`, `launch_quantization`, `legato`, `velocity_amount` — *are* exposed.) They **are** persisted in the `.als` project file, so the one programmatic route is editing it **offline** (Live closed), then reopening — verified against a Live 12.4.2 `.als`:
  - A `.als` is a single gzipped UTF-8 XML document. Round-trip: `gunzip -c p.als > p.xml` → edit → `gzip -nc p.xml > p.als` (re-gzip is byte-identical; `-n` matches how Live writes it). Live must be closed/reloaded to pick up the change.
  - Each clip (`… ClipSlot/Value/MidiClip/FollowAction`) and scene (`Scenes/Scene/FollowAction`) carries a `<FollowAction>` block: `FollowActionEnabled` (default `false` — set `true` to turn on), `FollowActionA`/`FollowActionB` (action enum), `FollowChanceA`/`FollowChanceB` (percent A:B), `FollowTime` (beats; 4 = 1 bar), `IsLinked`+`LoopIterations` (Linked: fire after N loops) vs `FollowTime` (Unlinked), `JumpIndexA`/`B` (target for the Jump action).
  - Action enum: `0` No Action · `1` Stop · `2` Play Again · `3` Play Previous · `4` Play Next (default) · `5` Play First · `6` Play Last · `7` Play Any · `8` Play Other · `9` Jump. (`JumpIndex` base 0-vs-1 unconfirmed.)
